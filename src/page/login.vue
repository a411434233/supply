        ...
        methods: {
            handleSubmit: function () {
                ...
                this.$http.post(store.state.config.apiUrlRoot + "user/login", this.userForm).then(response => {
                    ...
                    console.log(data);
                   if(data.code ==1 && data.content.roleLevel==1) {
                   this.$router.push({ path: '/module/user'});
                    that.$message({
                        message: data.msg,
                        type: 'success'
                    });
                    }
                   else if(data.code ==1 && data.content.roleLevel!=1){
                    	that.$message("对不起,你不是超级管理员!");
                    }
                   else{
